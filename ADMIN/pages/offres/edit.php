<?php 
	...
	if (isset($_GET['modif']) && !empty($_GET['modif'])) {
		...
		$req=$db->prepare("SELECT id_offre, offres.titre, categorie_offres.titre as type, description, profil_post, candidature, metiers, niveaux, experiences, lieu, logo, date_pub, date_exp, categorie_id, mem_id, etat FROM offres, categorie_offres WHERE  id_categorie = categorie_id AND id_offre =?");
  ...
  if ($taille<2097152) {
    ...
    if (isset($date_exp) && !empty($date_exp) && $date_exp!=$affichage['date_exp']) {
        
        $reqMiseAjour=$db->prepare("UPDATE offres SET date_exp=? WHERE id_offre=?");
        $reqMiseAjour->execute($date_exp,$modif);
      }

      if (isset($titre) && !empty($titre) && $titre!=$affichage['titre']) {
        
        $reqMiseAjour=$db->prepare("UPDATE offres SET titre=? WHERE id_offre=?");
        $reqMiseAjour->execute($titre,$modif);
      }

       if (isset($metiers) && !empty($metiers) && $metiers!=$affichage['metiers']) {
        
        $reqMiseAjour=$db->prepare("UPDATE offres SET metiers=? WHERE id_offre=?");
        $reqMiseAjour->execute($metiers,$modif);
      }

       if (isset($niveaux) && !empty($niveaux) && $niveaux!=$affichage['niveaux']) {
        
        $reqMiseAjour=$db->prepare("UPDATE offres SET niveaux=? WHERE id_offre=?");
        $reqMiseAjour->execute($niveaux,$modif);
      }

      if (isset($experience) && !empty($experience) && $experience!=$affichage['experiences']) {
        
        $reqMiseAjour=$db->prepare("UPDATE offres SET experiences=? WHERE id_offre=?");
        $reqMiseAjour->execute($experience,$modif);
      } 

      if (isset($lieu) && !empty($lieu) && $lieu!=$affichage['lieu']) {
        
        $reqMiseAjour=$db->prepare("UPDATE offres SET lieu=? WHERE id_offre=?");
        $reqMiseAjour->execute($lieu,$modif);
      }

       if (isset($description) && !empty($description) && $description!=$affichage['description']) {
        
        $reqMiseAjour=$db->prepare("UPDATE offres SET description=? WHERE id_offre=?");
        $reqMiseAjour->execute($description,$modif);
      }

        if (isset($profil) && !empty($profil) && $profil!=$affichage['profil_post']) {
        
        $reqMiseAjour=$db->prepare("UPDATE offres SET profil_post=? WHERE id_offre=?");
        $reqMiseAjour->execute($profil,$modif);
      }

      if (isset($candidature) && !empty($candidature) && $candidature!=$affichage['candidature']) {
        
        $reqMiseAjour=$db->prepare("UPDATE offres SET candidature=? WHERE id_offre=?");
        $reqMiseAjour->execute($candidature,$modif);
      }

       if (isset($categorie) && !empty($categorie) && $categorie!=$affichage['categorie_id']) {
        
        $reqMiseAjour=$db->prepare("UPDATE offres WHERE categorie_id=? WHERE id_offre=?");
        $reqMiseAjour->execute($categorie,$modif);
      }

      if (isset($nom_logo) && !empty($nom_logo) && $nom_logo!=$affichage['logo']) {
        
        $reqMiseAjour=$db->prepare("UPDATE offres WHERE logo=? WHERE id_offre=?");
        $reqMiseAjour->execute($nom_logo,$modif);
      }

      if(isset($modif)){

        $succes = "Félicitations! Les mises à jours de cette offre ont bien été prisent en compte |  <a href='?id_Us=$_SESSION[id_memoir]&p=gestion-offre'>Gestion</a>";
      ...
      // }else{
      // 	$erreur="Veuillez renplir les champs";
      // }
	}
 ?>
 <!-- bar de précision-->
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item">Offres</li>
           <li class="breadcrumb-item active">Edditer</li>
    </ol>
        <!-- Area Chart Example-->
    <div class="card mb-3">
        <div class="card-header text-white" style="background: #008080;">
              MODIFICATION DOS OFFRES</div><br>
 <div class="card-body">
<form method="POST" class="horizontal" enctype="multipart/form-data">
              ...
              ?>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    <input type="date" name="date_exp" class="form-control" value="<?= $affichage['date_exp']; ?>">
                  </div>
                  </div>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8"><input type="text" name="titre" class="form-control" value="<?= $affichage['titre']; ?>">
                    </div>
                  </div>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    <select name="categorie" class="form-control">
                      <option selected disabled value="<?= $affichage['type']; ?>"><?= $affichage['type']; ?></option>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    <select name="metiers[]"  multiple id="select">
                    	<option selected value="<?= $affichage['metiers']; ?>"><?= $affichage['metiers'] ?></option>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    <select name="niveaux[]" multiple id="select2">
                    	<option selected value="<?= $affichage['niveaux']; ?>"><?= $affichage['niveaux'] ?></option>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    ...
                    <option selected value="<?= $affichage['experiences']; ?>"><?= $affichage['experiences'] ?></option>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    <input type="text" name="lieu" class="form-control" value="<?= $affichage['lieu']; ?>">
                  </div>
                  </div>
                </div>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    <textarea name="description" class="form-control"><?= $affichage['description']; ?></textarea>
                  </div>
                  </div>
                </div>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    <textarea name="profil" class="form-control"><?= $affichage['profil_post']; ?></textarea>
                  </div>
                  </div>
                </div>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    <textarea name="candidature" class="form-control"><?= $affichage['candidature']; ?></textarea>
                  </div>
                  </div>
                </div>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    ...
                    <input type="text" disabled name="" value="<?= $affichage['logo']; ?>">
                  </div>
                  </div>
                </div>
